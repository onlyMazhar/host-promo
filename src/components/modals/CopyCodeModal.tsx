...
export default function CopyCodeModal({ deal, onClose }: CopyCodeModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg bg-white rounded-[12px] p-6 md:p-8 shadow-2xl border border-[#E5E7EB]">
        <button
          onClick={onClose}
        ...
        <div className="text-center space-y-4">
          ...
          {deal.promo_code && (
            <div className="p-4 bg-[#F5F5F6] rounded-[12px] border border-dashed border-[#FF2B85]/40 flex items-center justify-between gap-3">
              <span className="font-mono text-xl font-extrabold tracking-wider text-[#FF2B85]">
                {deal.promo_code}
              </span>
              <button
                onClick={handleCopyAgain}
                className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-white bg-[#343B46] hover:bg-black rounded-[12px] shadow-sm transition-all"
              >
          ...
          <div className="space-y-3 pt-2">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 px-6 btn-primary rounded-[12px] text-base"
            >
